...
// Entity는 요청이나 응답을 담는 값으로 쓰지 말라 했죠? -> DTO를 하나 만들어서 작업을 진행 
// 데이터 베이스에서 조회된 값을 화면으로 응답할 때, 해당 화면이 무엇이냐에 따라 
// 조회된 값을 가공하거나 추가하거나 제외하고 전달해야 할 필요성이 있기 때문에 
// 그게 맞는 응답 DTO를 생성해서 전달하는 것이 각자의 역할을 침해하지 않는 설계일 것입니다. 
@Getter
@ToString
@EqualsAndHashCode
public class ScoreListResponseDTO {
	...
	// 성은 *표시를 하지 않아요. -> 성만 먼저 빼자. -> charAt(0): 첫번째 인덱스 단일문자 빼기 -> 리턴타입: char -> String으로 변환하기: valueOf()
	// valueOf(): 다른 타입을 문자열로 변환하는 매서드
