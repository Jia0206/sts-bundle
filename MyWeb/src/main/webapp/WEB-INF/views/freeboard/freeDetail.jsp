﻿﻿<%@ page language="java" contentType="text/html; charset=UTF-8"
	...
	<script>
        window.onload = function() {
            ...
            보장이 없기 때문에 해당 방식은 이벤트 등록이 불가능 합니다. 
            이 때는 이미 실제로 존재하는 #replyList에 이벤트를 등록하고, 이벤트를 자식에게 
            위임하여 사용하는 addEventListener를 통해 처리해야 합니다. 
            */ 
            ...
            //수정 처리 함수, (수정 모달을 열어서 수정 내용을 작성한 후 수정 버튼ㅇ르 클릭했을 때 )
            document.getElementById('modalModBtn').onclick =() => {
                ...
                fetch('${pageContext.request.contextPath}/reply/' +rno.r.eqObj)
                    .then(res => res.text())
                    .then(data => {
                        if(data === 'pwFail'){
                            alert('비밀번호를 확인하세요!');
